...
@pytest.mark.parametrize("H_KV", [32])
...
def test_decode_attention(B, L, H_Q, H_KV, D_QK, D_V, CACHE_SIZE, PAGE_SIZE):
    ...
    req_to_token = req_to_token + torch.arange(PAGE_SIZE, device=device).view(
        1, 1, -1)
    req_to_token = req_to_token.view(B, -1)
    req_to_token = req_to_token[:, :seq_len].contiguous()

    # q represents the new token being generated, one per batch
    q = torch.randn(B, H_Q, D_QK, dtype=dtype, device=device)

    # k_buffer and v_buffer represent all previous tokens
    # Page size is 1.
    k_buffer = torch.randn(CACHE_SIZE, H_KV, D_QK, dtype=dtype, device=device)
    v_buffer = torch.randn(CACHE_SIZE, H_KV, D_V, dtype=dtype, device=device)
